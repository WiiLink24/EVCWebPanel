...
from app import app
from pollbooth.forms import LoginForm, NewUserForm, ChangePasswordForm
...
@app.route("/thepollbooth/login", methods=["GET", "POST"])
def login():
...
@app.route("/thepollbooth/create", methods=["GET", "POST"])
@login_required
def new_user():
    form = NewUserForm()
    if form.validate_on_submit():
...
@app.route("/thepollbooth/change_password", methods=["GET", "POST"])
@login_required
def change_password():
